...
* [INF-06] 확장성 (Auto-scaling) 부하 테스트: Locust를 이용한 1만 명 동시 접속 스파이크 테스트 수행. 스케일링 정책 임계치 50%로 축소, 대기열(Queue) 시스템 적용 및 인스턴스 Scale-out을 통해 서버 다운 없이 성공적으로 트래픽 방어.
...
* [INF-09] DB 특정 시점 복구 (PITR): 의도적인 DB 테이블 삭제 장애 유발 후, 트랜잭션 로그(WAL)를 활용해 데이터 유실 없이 삭제 직전 시점으로 완전 복원 성공.
...
### 1. 기술적 한계점
* **Provisioning Latency (프로비저닝 지연):** 현재 인프라는 VM 기동 시 실행되는 스크립트를 통해 도커 설치 및 환경 설정을 수행합니다. 신규 인스턴스가 서비스에 투입되기까지 약 3분가량의 지연이 발생하며, 이는 급격한 트래픽 스파이크 대응 시 가용성 리스크가 될 수 있음을 인지하고 있습니다.
* **단일 장애점(SPOF) 타협:** 비용 최적화(FinOps)를 위해 Redis를 `Basic` 티어로 구성하였습니다. 이는 장애 발생 시 자동 페일오버를 지원하지 않아, 시스템 전체의 단일 장애점으로 작용할 수 있는 설계상의 타협점입니다.

### 2. 실무 환경과의 차이점 인식
최대한 실무와 유사한 상황을 가정하여 검증을 진행했으나, 통제된 개인 프로젝트 환경인 만큼 실제 운영 환경과는 다음과 같은 분명한 차이가 존재함을 인지하고 있습니다.

* **통제된 장애와 예기치 못한 변수:** 준비된 상태에서 단일 장애를 주입한 본 테스트와 달리, 실제 장애는 예기치 못한 순간에 다발적이고 복합적으로 발생한다는 점에서 장애 대응 시의 압박감과 변수 통제에는 큰 차이가 있습니다.
* **라이브 데이터 무결성 및 병합:** 실제 대규모 사용자의 라이브 데이터가 부재하여, PITR 및 리전 페일오버 시 요구되는 정밀한 장애 시점 역추적, 데이터 동기화 및 무결성 검증, 그리고 부분 복구 후 운영 DB와의 병합 등 심도 있는 데이터베이스 운영 경험에는 한계가 있었습니다.
* **보안 관제 디테일 부재:** 트래픽 스파이크 및 DDoS 공격, 키 탈취 시나리오에서 방대한 실제 로그 속 외부 IP를 추적하고, 정탐과 오탐을 정교하게 분류해 내는 실무적인 보안 분석 프로세스는 온전히 경험하지 못했습니다.

### 3. 향후 개선 방안 및 포부 (Next Steps & Resolutions)
* **GKE 기반 아키텍처 마이그레이션:** 앞서 언급한 프로비저닝 지연 문제를 근본적으로 해결하기 위해, 향후 애플리케이션 환경을 MIG 기반에서 **GKE(Google Kubernetes Engine)** 기반의 컨테이너 오케스트레이션으로 마이그레이션할 계획입니다. 이를 통해 더욱 빠른 오토스케일링과 견고한 인프라를 실현하고자 합니다.